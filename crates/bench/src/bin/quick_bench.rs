...
fn main() {
    ...
    // IMPORTANT: free the raw `data` Vec before building to save 1.5-3 GB.
    // The normed copy is all build_segment needs.
    drop(data);
    // Pre-extract query vectors (reuse for search + recall).
    let nq = 200.min(n);
    ...
    let handles: Vec<_> = (0..cores).map(|_| {
        ...
        let queries = std::sync::Arc::clone(&queries_arc);
        let nq = qps_n / cores;
        std::thread::spawn(move || {
            for qi in 0..nq {
                let q = &queries[(qi % nq) * dim..(qi % nq) * dim + dim];
                let _ = vi.search_ef(q, 10, 128);
            }
        })
    }).collect();
